  ...
  app.get("/v1/account/account_info", async (req, res) => {
    try {
      ...
      if ((new Date().getTime() - new Date(user.token_last_refreshed).getTime()) > user.token_expires_in * 1000) {
        ...
        // Store
        await connection.query("UPDATE `users` SET `riot_access_token` = ?, `riot_refresh_token` = ?, `token_expires_in` = ?, `token_last_refreshed` = ? WHERE `riot_puuid` = ?", [newTokens.newAccessToken, newTokens.newRefreshToken, newTokens.expiresIn, new Date(), user.riot_puuid]);
      }
